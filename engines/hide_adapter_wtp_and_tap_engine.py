...
def train_one_epoch(model: torch.nn.Module, original_model: torch.nn.Module,
    ...
    for input, target in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        # if task_id > 0:
        #     with torch.no_grad():
        #         distill_output = model(input, task_id=task_id-1, train=set_training_mode)
        #         distill_feature = distill_output['pre_logits'].detach().clone()
        #         loss += distill_loss(output['pre_logits'], distill_feature)
...
def evaluate(model: torch.nn.Module, original_model: torch.nn.Module, data_loader,
    ...
    with torch.no_grad():
        for input, target in metric_logger.log_every(data_loader, args.print_freq, header):
            ...
            # if args.evaluate_type == 'tii+tap':
            #     adapter_id = torch.max(old_logits, dim=1)[1]
            #     # translate cls to task_id
            #     adapter_id = torch.tensor([target_task_map[v.item()] for v in adapter_id], device=device)
            #     output = model(input, task_id=adapter_id)
            #     logits = output['logits']
            #     logits = torch.max(logits, old_logits)

            # elif args.evaluate_type == 'topk_ensemble':
            #     old_logits = old_logits.softmax(dim=-1)
            #     conf, cls_id = torch.topk(old_logits, k=args.topk_ensemble)
            #     # conf = conf.softmax(dim=-1)
            #     ensemble_logits = []
            #     for i in range(args.topk_ensemble):
            #         adapter_id = torch.tensor([target_task_map[v.item()] for v in cls_id[:, i]], device=device)
            #         output = model(input, task_id=adapter_id)
            #         logits = output['logits']
            #         ensemble_logits.append(conf[:, i].unsqueeze(-1) * copy.deepcopy(logits))
            #     logits = torch.stack(ensemble_logits, dim=0).sum(dim=0)

            # elif args.evaluate_type == 'topp_ensemble':
            #     old_logits = old_logits.softmax(dim=-1)
            #     conf, cls_id = torch.topk(old_logits, k=args.topk_ensemble)
            #     conf[conf < args.topp_threshold] = 0
            #     ensemble_logits = []
            #     for i in range(args.topk_ensemble):
            #         adapter_id = torch.tensor([target_task_map[v.item()] for v in cls_id[:, i]], device=device)
            #         output = model(input, task_id=adapter_id)
            ...
            adapter_id = torch.max(old_logits, dim=1)[1]
            adapter_id = torch.tensor([target_task_map[v.item()] for v in adapter_id], device=device)
            task_inference_acc = utils.task_inference_accuracy(adapter_id.unsqueeze(-1), target, target_task_map)
    ...
    for task_id in range(args.num_tasks):
        ...
        if args.trained_original_model:
            ...
            if os.path.exists(original_checkpoint_path):
                ...
                return
        if args.trained_backbone_and_pet:
            checkpoint_path = os.path.join(args.output_dir, 'checkpoint/task{}_checkpoint.pth'.format(task_id + 1))
            if os.path.exists(checkpoint_path):
                print('Loading checkpoint from:', checkpoint_path)
                ckpt = torch.load(checkpoint_path, map_location=device)['model']
                state_dict = model_without_ddp.state_dict()
                not_in_k = [k for k in ckpt.keys() if k not in state_dict.keys() or 'head' in k or 'mlp' in k]
                for k in not_in_k:
                    del ckpt[k]
                state_dict.update(ckpt)
                model_without_ddp.load_state_dict(state_dict, strict=False)
                for n, p in model_without_ddp.named_parameters():
                    if n.startswith(tuple(args.freeze)) or 'adapter' in n:
                        p.requires_grad = False
            
        if task_id > 0 and not args.trained_backbone_and_pet:
            with torch.no_grad():
                if args.distributed:
                    model.module.adapters.down_proj.grad.zero_()
                    model.module.adapters.down_proj[task_id] = model.module.adapters.down_proj[task_id-1]
                    model.module.adapters.up_proj.grad.zero_()
                    model.module.adapters.up_proj[task_id] = model.module.adapters.up_proj[task_id-1]
                    model.module.adapters.down_bias.grad.zero_()
                    model.module.adapters.down_bias[task_id] = model.module.adapters.down_bias[task_id-1]
                    model.module.adapters.up_bias.grad.zero_()
                    model.module.adapters.up_bias[task_id] = model.module.adapters.up_bias[task_id-1]
                else:
                    model.adapters.down_proj.grad.zero_()
                    model.adapters.down_proj[task_id] = model.adapters.down_proj[task_id-1]
                    model.adapters.up_proj.grad.zero_()
                    model.adapters.up_proj[task_id] = model.adapters.up_proj[task_id-1]
                    model.adapters.down_bias.grad.zero_()
                    model.adapters.down_bias[task_id] = model.module.adapters.down_bias[task_id-1]
                    model.adapters.up_bias.grad.zero_()
                    model.adapters.up_bias[task_id] = model.module.adapters.up_bias[task_id-1]
        ...
        if args.adapter_momentum > 0 and task_id > 0 and not args.trained_backbone_and_pet:
            with torch.no_grad():
                model.module.adapters.down_proj[task_id].copy_(
                    (1 - args.adapter_momentum) * model.module.adapters.down_proj[task_id].detach().clone()
                    + args.adapter_momentum * model.module.adapters.down_proj[0:task_id].detach().clone().mean(dim=0))
                model.module.adapters.up_proj[task_id].copy_(
                    (1 - args.adapter_momentum) * model.module.adapters.up_proj[task_id].detach().clone()
                    + args.adapter_momentum * model.module.adapters.up_proj[0:task_id].detach().clone().mean(dim=0))
                model.module.adapters.down_bias[task_id].copy_(
                    (1 - args.adapter_momentum) * model.module.adapters.down_bias[task_id].detach().clone()
                    + args.adapter_momentum * model.module.adapters.down_bias[0:task_id].detach().clone().mean(dim=0))
                model.module.adapters.up_bias[task_id].copy_(
                    (1 - args.adapter_momentum) * model.module.adapters.up_bias[task_id].detach().clone()
                    + args.adapter_momentum * model.module.adapters.up_bias[0:task_id].detach().clone().mean(dim=0))
...
def distill_loss(features, distill_features):
    sim = torch.matmul(features / torch.norm(features, dim=0), (distill_features / torch.norm(distill_features, dim=0)).t()) / 0.8
    positive_sim = torch.diag(sim) 
    loss = -0.1 * torch.mean(positive_sim)
    return loss