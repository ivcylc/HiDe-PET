...
def evaluate(model: torch.nn.Module, original_model: torch.nn.Module, data_loader,
    ...
    with torch.no_grad():
        for input, target in metric_logger.log_every(data_loader, args.print_freq, header):
            ...
            with torch.no_grad():
                if original_model is not None:
                    output = original_model(input, use_mlp_head=args.use_mlp_head)
                    logits = output['logits']
                    if args.train_mask and class_mask is not None and not args.use_mlp_head:
                        mask = []
                        for id in range(task_id + 1):
                            mask.extend(class_mask[id])
                        not_mask = np.setdiff1d(np.arange(args.nb_classes), mask)
                        not_mask = torch.tensor(not_mask, dtype=torch.int64).to(device)
                        logits = logits.index_fill(dim=1, index=not_mask, value=float('-inf'))
                    if args.train_mask and class_mask is not None and args.use_mlp_head:
                        mask = []
                        for id in range(task_id + 1):
                            mask.extend(id)
            ...
            acc1, acc5 = accuracy(logits, target, topk=(1, 5))
            #acc1 = (torch.max(logits, dim=1)[1] == target).sum(0) * 100. / input.shape[0]
...
def train_and_evaluate(model: torch.nn.Module, model_without_ddp: torch.nn.Module, original_model: torch.nn.Module,
    ...
    for task_id in range(args.num_tasks):
        ...
        if args.trained_backbone_and_pet:
            ...
            if os.path.exists(checkpoint_path):
                ...
                for n, p in model_without_ddp.named_parameters():
                    if n.startswith(tuple(args.freeze)) or 'prompt' in n:
        ...
        if args.prompt_pool and args.shared_prompt_pool and not args.trained_backbone_and_pet:
            if task_id > 0:
                ...
                if (prev_end > args.size) or (cur_end > args.size):
                    ...
                    cur_idx = (slice(cur_start, cur_end)) if args.use_prefix_tune_for_e_prompt else (slice(cur_start, cur_end))
                    prev_idx = (slice(prev_start, prev_end)) if args.use_prefix_tune_for_e_prompt else (slice(prev_start, prev_end))
                    with torch.no_grad():
                        if args.distributed:
                            model.module.e_prompt.prompt.grad.zero_()
                            model.module.e_prompt.prompt[cur_idx].copy_(model.module.e_prompt.prompt[prev_idx].detach().clone())
                            # optimizer.param_groups[0]['params'] = model.module.parameters()
                        else:
                            model.e_prompt.prompt.grad.zero_()
                            model.e_prompt.prompt[cur_idx].copy_(model.e_prompt.prompt[prev_idx].detach().clone())
        ...
        if args.prompt_momentum > 0 and task_id > 0 and not args.trained_backbone_and_pet:
            if args.use_prefix_tune_for_e_prompt:
                with torch.no_grad():
                    print(model.module.e_prompt.prompt[task_id].shape)
                    print(
                        model.module.e_prompt.prompt[0:task_id].detach().clone().mean(dim=0, keepdim=True).shape)
        ...
        if actual_centroids != 0:
            print(f'actual centroid {actual_centroids}')
...
def _compute_mean(model: torch.nn.Module, data_loader: Iterable, device: torch.device, task_id, class_mask=None,
    ...
    for cls_id in class_mask:
        ...
        if args.ca_storage_efficient_method == 'multi-centroid':
            ...
            print(f'cluster {actual_cluster} centroids')
            actual_centroids += actual_cluster
...
def orth_loss(features, targets, device, args):
    if cls_mean:
        ...
        M = torch.cat([sample_mean, features], dim=0).to(device, non_blocking=True)
