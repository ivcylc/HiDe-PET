...
class StanfordDogs(torch.utils.data.Dataset):
    def __init__(self, root, split='train', transform=None, target_transform=None, download=False):
        self.fpath = os.path.join(root, 'stanford_dogs')
        self.train = split == 'train'
        if self.train:
            fpath = self.fpath + '/train'
        else:
...
class FGVCAircraft(datasets.FGVCAircraft):
    def __init__(self, root, split='trainval', annotation_level='variant', transform=None, target_transform=None, download=False):
        ...
        with open(labels_file, "r") as f:
            for line in f:
                ...
                self.data.append(os.path.join(image_data_folder, f"{image_name}.jpg"))
                self.targets.append(self.class_to_idx[label_name])
        self.classes = np.unique(self.targets)
