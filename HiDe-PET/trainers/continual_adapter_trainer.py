...
import vits.hide_adapter_vision_transformer as hide_adapter_vision_transformer
...
def train(args):
    ...
    if args.adapter_type == 'continual':
        from engines.continual_pet_engine import train_and_evaluate, evaluate_till_now
    if args.adapter_type == 'momentum':
        from engines.momentum_pet_engine import train_and_evaluate, evaluate_till_now

    if args.adapter_type == 'continual' and args.continual_type == 'slow_learner':
        base_params = [p for name, p in model_without_ddp.named_parameters() if 'adapter' in name and p.requires_grad == True]
        base_fc_params = [p for name, p in model_without_ddp.named_parameters() if 'adapter' not in name and p.requires_grad == True]
