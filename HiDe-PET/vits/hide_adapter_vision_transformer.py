...
from peft.adapter.hide_adapter import HideAdapter
from peft.adapter.continual_adapter import ContinualAdapter
from peft.adapter.momentum_adapter import MomentumAdapter
import logging
...
class Block(nn.Module):
    ...
    def forward(self, x, adapter=None, task_id=-1, depth_id=-1, train=False, old=False):
        residual = x
        x = self.drop_path1(self.ls1(self.attn(self.norm1(x))))
        if adapter is not None:
            x = adapter(x, task_id=task_id, depth_id=depth_id, train=train, old=old)
        x = residual + x
        # if adapter is not None:
        #     adapt_x = adapter(x, task_id=task_id, depth_id=depth_id, train=train)
...
class VisionTransformer(nn.Module):
    ...
    """
    def __init__(self, global_pool='token', class_token=True, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=True, drop_rate=0., attn_drop_rate=0., drop_path_rate=0., embed_layer=PatchEmbed, norm_layer=None,
                 act_layer=None, fc_norm=None, weight_init='', use_adapter=False, adapter=HideAdapter, continual_adapter=ContinualAdapter, pool_size=10, 
                 adapter_rank=8, adapter_depth=5, adapter_type='hide', momentum_adapter=MomentumAdapter,
                 mlp_structure=[], use_mlp_head=False, mlp_output_dim=10):
        super().__init__()
        ...
        if use_adapter:
            if adapter_type == 'hide':
                self.adapters = adapter(pool_size=pool_size, depth=adapter_depth, dim=embed_dim, rank=adapter_rank)
            elif adapter_type == 'continual':
                self.adapters = continual_adapter(depth=adapter_depth, dim=embed_dim, rank=adapter_rank)
            elif adapter_type == 'momentum':
                self.adapters = momentum_adapter(depth=adapter_depth, dim=embed_dim, rank=adapter_rank)
        else:
            self.adapters = None
    ...
    def forward_features(self, x, train=False, task_id=-1, old=False):
        B = x.shape[0]
        ...
        for i in range(self.depth):
            if self.use_adapter and i < self.adapter_depth:
                if self.grad_checkpointing and not torch.jit.is_scripting():
                    x = checkpoint_seq(self.blocks[i], x, adapter=self.adapters, task_id=task_id, depth_id=i, train=train, old=old)
                else:
                    x = self.blocks[i](x, adapter=self.adapters, task_id=task_id, depth_id=i, train=train, old=old)
            else:
                if self.grad_checkpointing and not torch.jit.is_scripting():
    ...
    def forward(self, x, train=False, task_id=-1, fc_only=False, old=False, use_mlp_head=False, mlp_head_only=False):
        res = dict()
        if fc_only:
            if not use_mlp_head:
                x = self.mlp(x)
        ...
        res = self.forward_features(x, train=train, task_id=task_id, old=old)
        
        res = self.forward_head(res, use_mlp_head=use_mlp_head)
        return res
    
    def after_task(self, task_id=-1, device=None):
        if self.adapter_type in ['continual', 'momentum']:
            self.adapters.after_task(task_id, device)
...
def vit_base_patch16_224_21k_ibot(pretrained=False, **kwargs):
    ...
    # del model.head
    state_dict = model.state_dict()
    s_ckpt = torch.load('/data1/data/checkpoint.pth', map_location='cpu')['teacher']
    ckpt = {}
...
def vit_small_patch16_224_ims(pretrained=False, **kwargs):
    ...
    #del model.head
    state_dict = model.state_dict()
    ckpt = torch.load('/data1/data/best_checkpoint.pth', map_location='cpu')['model']
    ckpt_keys = ckpt.keys()
    not_in_k = [k for k in ckpt.keys() if k not in state_dict.keys()]
    head = [k for k in ckpt.keys() if 'head' in k]
    for k in head:
