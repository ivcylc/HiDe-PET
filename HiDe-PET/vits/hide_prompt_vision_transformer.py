...
from peft.prompt.momentum_prompt import MomentumPrompt
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            mlp_structure=[], prompt_type='hide', use_mlp_head=False, mlp_output_dim=10):
        """
        Args:
        ...
        if use_e_prompt and e_prompt_layer_idx is not None:
            ...
            if prompt_type == 'continual':
                self.e_prompt = ContinualPrompt(length=prompt_length, embed_dim=embed_dim, num_layers=num_e_prompt, use_prefix_tune_for_e_prompt=use_prefix_tune_for_e_prompt, num_heads=num_heads)
            if prompt_type == 'momentum':
                self.e_prompt = MomentumPrompt(length=prompt_length, embed_dim=embed_dim, num_layers=num_e_prompt, use_prefix_tune_for_e_prompt=use_prefix_tune_for_e_prompt,
                                        num_heads=num_heads)
    ...
    def forward_features(self, x, task_id=-1, prompt_id=None, train=False, prompt_momentum=0, old=False):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            if self.use_e_prompt:
                ...
                if self.prompt_type in ['continual', 'momentum']:
                    res = self.e_prompt(x, train=train, old=old)                
    ...
    def forward(self, x, task_id=-1, prompt_id=None, prompt_weight=None, train=False, fc_only=False, prompt_momentum=0, old=False, use_mlp_head=False, mlp_head_only=False):
        if fc_only:
            res = dict()
            if not use_mlp_head:
                x = self.mlp(x)
                ...
                if mlp_head_only:
                    ...
                    res['logits'] = self.mlp_head(x)
                return res
        res = self.forward_features(x, task_id=task_id, prompt_id=prompt_id, train=train, old=old)
