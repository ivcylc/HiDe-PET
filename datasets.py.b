...
def get_dataset(dataset, transform_train, transform_val, args, target_transform=None):
    if dataset == 'CIFAR100':
        dataset_train = datasets.CIFAR100(args.data_path, train=True, download=True, transform=transform_train, target_transform=target_transform)
        dataset_val = datasets.CIFAR100(args.data_path, train=False, download=True, transform=transform_val, target_transform=target_transform)
        ...
        dataset_train = Flowers102(args.data_path, split='train', download=True, transform=transform_train, target_transform=target_transform)
        dataset_val = Flowers102(args.data_path, split='test', download=True, transform=transform_val, target_transform=target_transform)
        ...
        dataset_train = StanfordDogs(args.data_path, split='train', download=True, transform=transform_train, target_transform=target_transform).data
        dataset_val = StanfordDogs(args.data_path, split='test', download=True, transform=transform_val, target_transform=target_transform).data
        ...
        dataset_train = Imagenet_R(args.data_path, train=True, download=True, transform=transform_train, target_transform=target_transform)
        dataset_val = Imagenet_R(args.data_path, train=False, download=True, transform=transform_val, target_transform=target_transform)
    
    elif dataset == 'Aircraft':
        dataset_train = FGVCAircraft(args.data_path, split='trainval', download=True, transform=transform_train, target_transform=target_transform)
        dataset_val = FGVCAircraft(args.data_path, split='test', download=True, transform=transform_val, target_transform=target_transform)
...
def split_multiple_dataset(datasets_info, args):
    ...
    for name, dataset in datasets_info.items():
        ...
        for i in range(dataset['num_tasks']):
            ...
            for k in range(len(dataset['train'].targets)):
                ...
                if int(dataset['train'].targets[k]) + last_index - dataset['num_continual_classes'] in scope:
                    train_split_indices.append(k)
            

            for h in range(len(dataset['val'].targets)):
                # if int(dataset['val'].targets[h]) + last_index in scope:
                if int(dataset['val'].targets[h]) + last_index - dataset['num_continual_classes'] in scope:
                    test_split_indices.append(h)
            ...
            cls_split_datasets.update(split_multiple_class_dataset(dataset['cls_train'], dataset['cls_val'], [scope], last_index, args, dataset['num_continual_classes']))
    ...
    print(mask)
    print(cls_split_datasets)

    tasks = [i for i in range(num_tasks)]
    if args.shuffle:
        random.shuffle(tasks)
    args.tasks_order = tasks

    shuffle_split_datasets = []
    shuffle_cls_split_datasets = {}
    shuffle_mask = []
    ...
    return cls_split_datasets, shuffle_split_datasets, shuffle_mask, target_dataset_map, target_task_map, shuffle_task_dataset_map

def split_multiple_class_dataset(dataset_train, dataset_val, mask, last_index, args, num_continual_nums=0):
    nb_classes = len(dataset_val.classes)
    print(nb_classes)
    split_datasets = dict()
    print(mask)
    for i in range(len(mask)):
        ...
        for cls_id in single_task_labels:
            ...
            for k in range(len(dataset_train.targets)):
                if int(dataset_train.targets[k]) + last_index - num_continual_nums == cls_id:
                    train_split_indices.append(k)

            for h in range(len(dataset_val.targets)):
                if int(dataset_val.targets[h]) + last_index - num_continual_nums == cls_id:
                    test_split_indices.append(h)
...
def build_upstream_continual_dataloader(args):
    ...
    for i, dataset in enumerate(args.datasets):
        ...
        dataset_train, dataset_val = get_dataset(dataset.replace('Split-', ''), transform_train, transform_val,
                                                 args, target_transform=partial(target_transform, nb_classes=last_classes_index-len(args.continual_datasets_targets[i])))
        dataset_train_mean, dataset_val_mean = get_dataset(dataset.replace('Split-', ''), transform_val,
                                                           transform_val, args, target_transform=partial(target_transform, nb_classes=last_classes_index-len(args.continual_datasets_targets[i])))
        ...
        datasets_info[i]['num_classes'] = len(args.fs_datasets_targets[i])
        datasets_info[i]['num_tasks'] = args.tasks_per_dataset[i]
        datasets_info[i]['cls_train'] = dataset_train_mean
        datasets_info[i]['cls_val'] = dataset_val_mean
        datasets_info[i]['num_continual_classes'] = len(args.continual_datasets_targets[i])
        last_classes_index += datasets_info[i]['num_classes']

    cls_split_datasets, splited_dataset, class_mask, target_dataset_map, target_task_map, task_dataset_map = split_multiple_dataset(datasets_info, args)
    ...
    for i in range(len(class_mask)):
        for cls_id in class_mask[i]:
            dataset_train_cls, dataset_val_cls = cls_split_datasets[cls_id]
    ...
    return dataloader_per_cls, dataloader, class_mask, target_dataset_map, target_task_map, task_dataset_map
